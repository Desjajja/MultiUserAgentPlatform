...
import { getInboundSourceSessionId, insertMessage, migrateMessagesInTable } from './session-db.js';
...
describe('migrateMessagesInTable', () => {
  ...
  it('adds traceparent on a legacy DB, leaves existing rows NULL, is idempotent', () => {
    if (fs.existsSync(TEST_DIR)) fs.rmSync(TEST_DIR, { recursive: true });
    fs.mkdirSync(TEST_DIR, { recursive: true });
    ...
    const cols = (db.prepare("PRAGMA table_info('messages_in')").all() as Array<{ name: string }>).map((c) => c.name);
    expect(cols).toContain('traceparent');

    const row = db.prepare('SELECT traceparent FROM messages_in WHERE id = ?').get('legacy-3') as {
...
describe('insertMessage', () => {
  it('persists traceparent for inbound rows', () => {
    if (fs.existsSync(TEST_DIR)) fs.rmSync(TEST_DIR, { recursive: true });
    fs.mkdirSync(TEST_DIR, { recursive: true });
    ...
    insertMessage(db, {
      id: 'msg-traceparent-1',
      kind: 'chat',
      timestamp: new Date().toISOString(),
      platformId: null,
      channelType: 'agent',
      threadId: null,
      content: JSON.stringify({ text: 'delegated turn' }),
      processAfter: null,
      recurrence: null,
      traceparent: '00-0123456789abcdef0123456789abcdef-fedcba9876543210-01',
    });

    const row = db.prepare('SELECT traceparent FROM messages_in WHERE id = ?').get('msg-traceparent-1') as {
      traceparent: string | null;
    };
    expect(row.traceparent).toBe('00-0123456789abcdef0123456789abcdef-fedcba9876543210-01');
    db.close();
  });
});
