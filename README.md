...
# Frontdesk 分支默认策略（见 ADR-0019）
# main / 未知分支：默认 frontlane-template-frontdesk
# dxy-dev：未设置 ENTERPRISE_FRONTDESK_FOLDER 时默认 frontlane-lab-frontdesk
# ENTERPRISE_FRONTDESK_FOLDER=frontlane-lab-frontdesk  # 显式覆盖，优先级最高
```
...
`frontlane-template-frontdesk` 仍是 `pnpm init:enterprise` 的 primary/template desk；不要把 `DEFAULT_FRONTDESKS[0]` 改成 lab。`dxy-dev` 的本地运行时默认由 `src/branding.ts` 的分支策略切到 `frontlane-lab-frontdesk`。合入前可运行：
...
- [docs/SPEC.md](docs/SPEC.md) — 平台定位 + 不做什么的边界 + ADR-0020 扩展
...
- [docs/specs/architecture.md](docs/specs/architecture.md) — 架构详细设计
- [docs/specs/observability.md](docs/specs/observability.md) — 可观测性完整 spec（span 命名、业务标签、trace 拓扑、ADR-0020 扩展）
