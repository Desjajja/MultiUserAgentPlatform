...
| 2 | **本地 raw 文件目录** | `~/.openclaw/workspace-exec-labops/archives/{exp_id}/raw/` | Chromeleon 导出的 csv、原始图片、动作日志等原始数据文件 | 查阅完整原始记录 |
| 3 | **RAG 知识库 + 本地 summary** | POST `http://localhost:7001/api/knowledge/upload`（RAG）<br>同时落盘 `~/.openclaw/workspace-exec-labops/archives/{exp_id}/summary.md`（本地副本） | LLM 生成的实验总结 markdown | "找类似实验"语义搜索 |
...
```bash
mkdir -p ~/.openclaw/workspace-exec-labops/archives/<exp_id>/raw/
cp <原始文件1> <原始文件2> ... ~/.openclaw/workspace-exec-labops/archives/<exp_id>/raw/
```
...
- summary.md 必含板块：
  ...
  - 改进建议
- **双写**：
  - 落盘：`~/.openclaw/workspace-exec-labops/archives/<exp_id>/summary.md`
