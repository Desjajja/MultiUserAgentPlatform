...
详细 schema：
- [docs/specs/db.md](specs/db.md) — 三库总览 + 跨挂载不变式
- [docs/specs/db-central.md](specs/db-central.md) — Central DB 表逐个解释
- [docs/specs/db-session.md](specs/db-session.md) — 两文件 session 表 + seq 奇偶约定
...
**深入**：[docs/specs/architecture.md "Identity Model"](specs/architecture.md) + `container/agent-runner/src/request-identity.ts` + `request-context.ts`。
...
**深入**：[docs/specs/architecture.md "Container Properties"](specs/architecture.md) + `src/host-sweep.ts:decideStuckAction()` + `src/container-runner.ts`。
...
要接其他平台（Slack / WhatsApp / Telegram）需自己写 adapter，参考 `src/channels/feishu.ts`。Channel 协议在 [docs/specs/api-details.md](specs/api-details.md)。
...
| `ENTERPRISE_FRONTDESK_FOLDER` | 分支默认 | 显式指定 autowire frontdesk；未设置时 `main`/未知使用 template，`dxy-dev` 使用 lab |
...
| [docs/SPEC.md](SPEC.md) | 平台定位 + 不做什么的边界 + ADR-0020 扩展（模型注册表、上下文可见性、成本追踪、Prompt 版本追踪） |
| [docs/specs/architecture.md](specs/architecture.md) | 详细的 message flow + identity model + design decisions |
| [docs/specs/db.md](specs/db.md) | 三 DB 模型、跨挂载 invariants |
| [docs/specs/db-central.md](specs/db-central.md) | Central DB 每张表 |
| [docs/specs/db-session.md](specs/db-session.md) | inbound/outbound 表 + seq 奇偶 |
...
| [docs/specs/api-details.md](specs/api-details.md) | Channel adapter / inbound / outbound 内容格式 |
| [docs/specs/agent-runner-details.md](specs/agent-runner-details.md) | 容器侧 poll-loop / formatter / MCP tool 接口 |
| [docs/specs/build-and-runtime.md](specs/build-and-runtime.md) | Node host + Bun container 双 runtime 的 lockfile / image build |
| [docs/specs/observability.md](specs/observability.md) | 可观测性完整 spec：span 命名、OpenInference 属性、业务标签、trace 拓扑、ADR-0020 扩展 |
