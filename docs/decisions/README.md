...
| [ADR-0014](ADR-0014-observability-span-schema.md) | Observability Span Naming Schema v1.0（hierarchical snake_case + OpenInference attribute matrix + 20 namespaces + 5 LOCKED decisions；spec 已合并至 `docs/specs/observability.md`） | Accepted | 2026-05-31 | `observability`, `phase0b`, `tracing`, `naming` |
...
| [ADR-0016](ADR-0016-litellm-proxy-gateway.md) | LiteLLM Proxy Gateway 引入（observability compose stack 扩展 + PR-O3 container OTel + llm-usage 数据黑洞消除） | Accepted | 2026-06-01 | `observability`, `phase0b`, `provider`, `infra`, `litellm` |
| [ADR-0017](ADR-0017-observability-phoenix17-route-type-taxonomy.md) | Phoenix 17.2.0 升级 + 业务过滤维度迁移到 OpenInference `metadata` + `route_type` taxonomy 重构（删 `A2A` 加 `ERP` reserved + host→container route context 传播）— Phoenix 升级部分继续有效；route-type-first filter 语义被 ADR-0018 超越 | Accepted（部分被 ADR-0018 超越） | 2026-06-07 | `observability`, `phase0b`, `tracing`, `phoenix`, `schema-governance` |
| [ADR-0018](ADR-0018-observability-business-first-tracing.md) | Business-First Tracing — `interaction.*` 根 span、`span_scope` taxonomy、平台/工具 span 隔离（超越 ADR-0017 的 route-type-first 过滤语义） | Accepted | 2026-06-09 | `observability`, `phase0b`, `tracing`, `phoenix`, `schema-governance` |
| [ADR-0019](ADR-0019-branch-default-frontdesk-policy.md) | Branch-Default Frontdesk Policy — `main`/unknown keep template, `dxy-dev` defaults runtime QA to lab, env override wins | Accepted | 2026-06-10 | `naming`, `governance`, `frontdesk`, `phase0a` |
| [ADR-0020](ADR-0020-model-metadata-registry.md) | 模型元数据注册表 + 上下文可见性 + 推理/成本追踪 — YAML Profile Registry + Phoenix context visibility + reasoning content capture | Accepted | 2026-06-10 | `observability`, `provider`, `model-registry`, `cost-tracking`, `reasoning`, `context-visibility`, `phoenix` |
