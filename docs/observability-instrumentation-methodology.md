...
## 10. 业务标签过滤（Business Tags）

### 10.1 背景

所有 MUAP spans 携带 `muap.*` 命名空间的业务标签，用于在 Phoenix 中按业务维度过滤：

| 标签键 | 合法值 | 设置对象 |
|---|---|---|
| `muap.layer` | `ai` / `platform` | 所有 span |
| `muap.route_type` | `frontdesk` / `worker` / `a2a` / `system` | 所有 span |
| `muap.lane` | `frontdesk` / `worker` | 路由/车道相关 span |
| `muap.channel` | `cli` / `feishu` / `webhook` | ingress span |
| `muap.intent` | `chat` / `approval` / `execute` / `system` | agent span |
| `muap.provider` | `claude` / `openai` / `sdk-openai` | 容器/provider span |

### 10.2 Phoenix 查询语法

Phoenix 将点号分隔的属性键自动嵌套为 JSON object，因此查询时必须用嵌套路径：

```sql
-- 正确：查询 AI 语义层的 spans
SELECT name, span_kind FROM spans
WHERE attributes->'muap'->>'layer' = 'ai';

-- 错误：flat 路径在 Phoenix 中返回空
-- WHERE attributes->>'muap.layer' = 'ai'  -- 不要这样写
```

UI Filter Bar（Spans 标签页）：
```
...
- **AI Semantic Layer**（`muap.layer = 'ai'`）：`router.deliver_to_agent`（AGENT）、`agent.turn`（CHAIN）、LiteLLM auto-spans（LLM/TOOL）
- **Platform Telemetry Layer**（`muap.layer = 'platform'`）：`channel.*.receive`、`router.route`、`container.wake/spawn/kill`、`delivery.*`

平台 spans 继续保留在 trace tree 中，但通过 `muap.layer` 可在 Phoenix 中独立过滤。

## 11. 验证 SQL（Phoenix Postgres）
...
-- 6. agent.turn span 已到达（v1.2 新增）
SELECT
  name,
  span_kind,
  attributes->'muap'->>'layer' AS layer,
  attributes->'muap'->>'route_type' AS route_type,
  attributes->'muap'->>'provider' AS provider
FROM spans
WHERE name = 'agent.turn'
ORDER BY start_time DESC
LIMIT 5;
-- 期望：能看到 agent.turn，span_kind='CHAIN'，layer='ai'，route_type='worker'

-- 7. 业务标签过滤验证（v1.2 新增）
SELECT name, span_kind, attributes->'muap'->>'layer' AS layer
FROM spans
WHERE attributes->'muap'->>'layer' IS NOT NULL
ORDER BY start_time DESC
LIMIT 10;
-- 期望：所有最近 spans 都有 muap.layer（ai 或 platform）
```

UI 验证：Phoenix Sessions 视图打开 → HUMAN 卡片显示用户消息文本 / AI 卡片显示回复文本，不再 `undefined`。Spans 标签页 filter bar 可过滤 `attributes["muap.layer"].as_string() == "ai"`。
...
- **ADR-0016**: `LiteLLM Proxy Gateway 引入`（PR-O3 container OTel + LLM 可观测性统一）

ADR 在 PR-O2.1 实施 commit 中一并落地。ADR-0016 在 PR-O3 实施 commit 中落地。
