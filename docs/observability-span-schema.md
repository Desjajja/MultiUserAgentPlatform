...
| `router.*` | inbound routing / session resolution / delegation decisions | `CHAIN` | routing spine；`router.deliver_to_agent` is current session-trace root | Active | `router.deliver_to_agent` | `src/router.ts` |
...
|---|---|---|---|---|
| Root spans（current session roots） | `CHAIN` | `session.id`, `user.id`, `input.value`, `input.mime_type` | `output.value`, `output.mime_type` if root also emits user-visible output | current canonical root is `router.deliver_to_agent` |
...
当前 locked rule：
> `router.deliver_to_agent` is the OTel root for session traces.
